...
def exif_size(img):
    ...
    try:
        rotation = dict(img._getexif().items())[orientation]#调整数码相机照片的方向
...
class LoadImages:
    ...
    def __init__(self, path, img_size=640, stride=32, auto=True):
        ...
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
            ...
            elif os.path.isdir(p):#如果是文件夹，使用glob获取全部文件路径
        ...
        # x.split获取文件名和后缀
        # 分别提取图片和视频文件路径
        images = [x for x in files if x.split('.')[-1].lower() in IMG_FORMATS]#检查是否是支持的图片类型
        videos = [x for x in files if x.split('.')[-1].lower() in VID_FORMATS]
        # 获取图片和视频的数量
        ni, nv = len(images), len(videos)

        self.img_size = img_size#输入图片的size
        self.stride = stride
        self.files = images + videos#整合图片和视频路径到一个列表
        self.nf = ni + nv  # number of files；总的文件数
        self.video_flag = [False] * ni + [True] * nv
        # 初始化模块信息，代码中对于mode=images与mode = video有不同的处理
        self.mode = 'image'
        self.auto = auto
    ...
    def __next__(self):
        ...
        img = np.ascontiguousarray(img) #数组转换成内存连续，提高运行速度
...
# 定义迭代器
# cv2视频读取函数
# cap.grap（）从设备或视频获取下一帧，获取成功返回True否则返回false
# cap.retrieve（frame）在grab后使用，对获取的帧进行解码，也返回true或false
    ...
    def __init__(self, sources='streams.txt', img_size=640, stride=32, auto=True):
        ...
        if os.path.isfile(sources):
            with open(sources) as f:
                sources = [x.strip() for x in f.read().strip().splitlines() if len(x.strip())]#如果是文件读取文件中的每一行
        else:
            sources = [sources]
        ...
        for i, s in enumerate(sources):  # index, source
            ...
            s = eval(s) if s.isnumeric() else s  # i.e. s = '0' local webcam ；如果source = 0 打开摄像头，否则打开视频流地址
            if s == 0:
...
# 自定义的数据集
# 定义LoadImagesAndLabels类，继承Dataset，重写抽象方法__len()__,__getitem()__
class LoadImagesAndLabels(Dataset):
    ...
    def __init__(self,
            ...
            for p in path if isinstance(path, list) else [path]:
                ...
                if p.is_dir():  # dir
                    ...
                    with open(p) as t:
                        ...
                        # 分隔符替换为os.sep,os.splitext(x)将文件名和扩展名分开并返回一个列表
                        parent = str(p.parent) + os.sep
                        f += [x.replace('./', parent) if x.startswith('./') else x for x in t]  # local to global path
                        # f += [p.parent / x.lstrip(os.sep) for x in t]  # local to global path (pathlib)
                else:
                    raise FileNotFoundError(f'{prefix}{p} does not exist')
            self.im_files = sorted(x.replace('/', os.sep) for x in f if x.split('.')[-1].lower() in IMG_FORMATS)#sorted对图片进行排序
        ...
        # Check cache
        self.label_files = img2label_paths(self.im_files)  # labels#返回labels的路径
        ...
        if self.rect:
            ...
            irect = ar.argsort()#获取ar从小到大的排序索引
            # 根据索引和排序数据集与标签路径，shape，h/w
    ...
    def __len__(self):
        return len(self.im_files)#返回训练集的图片的个数
    ...
    def __getitem__(self, index):
        ...
        if mosaic:
            ...
            if labels.size:  # normalized xywh to pixel xyxy format
                # 调整pad调整框的标签坐标，并从归一化的xywh-->未归一化的xyxy
...
# 生成一个mosaic增强的图片
    def load_mosaic(self, index):
        ...
        for i, index in enumerate(indices):
            # Load image
            # 加载图片并根据设定的输入大小与原图片的比例ratio进行resize
        ...
        # 进行moosaic的时候四张图片整合到一起以后shape为[2*img_size,2*img_size]
        # 对mosaic整合的图片随机旋转，平移，缩放，裁剪，并resize为输入大小img_size
        # Augment
    ...
    @staticmethod
    def collate_fn(batch):# 整理函数，如何取样本
